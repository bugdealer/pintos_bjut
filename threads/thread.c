...
static int mlfqs_queue_size; //多级反馈队列中进程的个数
  /***************************/
	...
	thread_tick (void) 
{
	...
	if(thread_mlfqs)
	{
		...
		if(timer_ticks() % 4== 0)
		{
			thread_compute_priorities();  //计算优先级
		}
	
		if(timer_ticks() % 100 == 0)
		{
			thread_compute_recent_cpu();    //计算recent_cpu
			thread_compute_load_average();  //计算load_average
		} 
	}
	  /***************************/
...
void thread_unblock (struct thread *t) 
{
   ...
   /***************************/
	if(thread_mlfqs)
		thread_add_to_queue(t); //插入多级反馈队列
	else
		list_insert_ordered(&ready_list, &t->elem, thread_priority_func, NULL); //插入read_list
	...
	thread_set_priority (int new_priority) 
{
	...
	curr->priority = new_priority;
	  ...
	  /***************************/ 
	if(!list_empty(&ready_list))  //设置优先级后，对比是否ready_list第一个进程优先级大于当前优先级
	{
	...
	next_thread_to_run (void) 
{
	...
	if(thread_mlfqs)
	{
		if(mlfqs_queue_size == 0)
		{
			...
			return thread_pop_max_priority_list();  //发生调度后，拿出多级反馈队列中优先级最高的
		}
	}
	...
	else
	{
		if (list_empty (&ready_list))
			...
			return list_entry (list_pop_front (&ready_list), struct thread, elem);  //调度发生，拿出队头优先级最高的
	}
	 /***************************/
}
...
{
	int priority = t->priority;    //获取线程当前优先级
	if (!list_empty(&t->rec_donations)) { //查看该线程捐赠队列是否为空，如果不空，则从中选择一个比原优先级大的作为返回，若没有返回原优先级
...
void thread_sleep(int64_t ticks)  //线程睡眠
{
	struct thread * cur=thread_current();
	ASSERT(!intr_context());
...
void
	thread_reinsert_list(struct thread *t, struct list *list)  //一个线程的优先级发生变化（可能是被捐赠后），重新插入ready_list
{
...
void add_thread_donation(struct lock * t,struct thread* cur)  //添加捐赠
{
	bool found=0;
	...
	while(lock!=NULL)
	{
		...
		if(found) //找到因为当前的锁捐赠元素，如果新的优先级尕高于原有的优先级，修改其原有的优先级
		{
